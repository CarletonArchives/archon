...
<form action="index.php" accept-charset="UTF-8" method="get" onsubmit="if(!this.q.value) { alert('<?php echo($strPleaseEnter); ?>'); return false; } else { return true; }">
   <div id="dlsearchblock">
      ...
      <div class='radiobuttonset'>
        ...
        <input type='radio' class='fieldradiobutton' id='radioDocument' name='media' value='document' /> <label for='radioDocument'>Documents</label>
        <input type='radio' class='fieldradiobutton' id='radioAll' name='media' value='all' /> <label for='radioAll'>All</label>
      </div>
   </div>
</form>
</div>
...
if(!empty($arrDigitalContent))
{
    ...
    foreach($arrDigitalContent as $objDigitalContent)
    {
        ...
        foreach($objDigitalContent->Files as $objFile)
        {
            ...
            elseif ($media == 'Video') {
                $content = "<video class='videothumbnail' muted preload='none' onmouseover='this.setAttribute(\"src\",\"". $mediaURL ."#t=8\"); this.play()' onmouseout='this.setAttribute(\"src\",\"\");this.load()'>";
            ...
            <div class="<?php echo($cssClass); ?>">
                ...
                <div class="thumbnailcaption">
                    ...
                    {
                        echo '(' . $count . ' out of ' . count($objDigitalContent->Files) . ')';
                    }
                    ?>
                </div>
            </div>
...
$(document).ready(function(){
  ...
  $(".videothumbnail").each(function(i, vid) {
    ...
    vid.addEventListener('seeked', function handler() {
      ...
      canvas.setAttribute("width",vid.videoWidth);// * scale;
      canvas.setAttribute("height",vid.videoHeight);// * scale;
