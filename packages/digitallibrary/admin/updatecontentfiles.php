...
//Current Draft of Warning: You have files stored in your database. This tool is only designed to work with links.
//If you would still like to use this tool, you can either use the blob zapper to create links to replace files,
//or you can contact somebody about changing this tool.
/*
...
foreach ($rows as $objDigitalContent) {
    ...
    foreach($newFileArr as $url => $size) {
        ...
        $file->TempFileName = basename($url);
        $file->Size = $size;
		echo($file->ID);
        if ($file->dbStore()) {
            $uploads++;
            ...
            $numerrs++;
            echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"' . $file->Filename . '" could not be uploaded. ' . $_ARCHON->Error . '<br>';
        }
    }
